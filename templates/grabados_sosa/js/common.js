jQuery(function()
{
	...
	{
		var myLatLng = map_location2 || {lat: -16.497918, lng: -68.134592};
		...
		var infowindow = new google.maps.InfoWindow({
		    content: map_location2.address || 'No hay informacion disponible'
		});
		infowindow.open(map, marker);
	}
	if( jQuery("#map3").length > 0)
	{
		var myLatLng = map_location3 || {lat: -16.497918, lng: -68.134592};
		...
		var infowindow = new google.maps.InfoWindow({
		    content: map_location3.address || 'No hay informacion disponible'
		});
		infowindow.open(map, marker);
	}
	if( jQuery().lightGallery)
	{
		jQuery('.products-list').lightGallery({
			selector: '.image'
		});
	}
