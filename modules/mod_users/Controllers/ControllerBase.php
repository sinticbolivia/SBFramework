...
abstract class ControllerBase extends SB_Controller
{
	/**
	 * 
	...
	public function task_recover_pwd_now()
	{
		...
		$email = $this->dbh->EscapeString($email);
		$user = $this->usersModel->GetBy('email', $email);
		if( !$user )
		{
		...
		//$user 	= $this->usersModel->Get($ruser->user_id);
	...
	public function task_upload_image()
	{
		...
		try
		{
			...
			if( sb_is_user_logged_in() && $this->request->getInt('update') )
			{
		...
		{
			sb_response_json(['status' => 'error', 'error' => $e->getMessage()]);
		}
