...
class SB_TableList extends SB_Object
{
	protected	$table;
	/**
	...
	public function FillFromArray($items)
	{
		$export				= SB_Request::getString('export');
		$this->items 		= $items;
		$this->columns 		= array();
			...
			<div class="table-responsive">
				...
				<tr <?php foreach($item as $prop => $value){print "data-$prop=\"$value\" ";}?>>
					...
					<td class="col-actions">
						...
						<?php
                        $continue = isset($data['callback']) ? 
                                        call_user_func($data['callback'], $_action, $data, $item, $this) : 
                                        true;
                        if( !$continue )
                            continue;
						$action = $_action;
