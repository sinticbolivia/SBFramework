...
class SB_Image
{
	...
	public function load($image_file)
	{
		...
		if( !$this->image_info )
		{
			throw new Exception('[SB_Image]: Invalid image info "'. $image_file .'"');
		}
	 ...
	 * @param int $image_type The image type IMAGETYPE_JPEG|IMAGETYPE_GIF|IMAGETYPE_PNG
	...
	function save($filename, $image_type = null, $compression = 75, $permissions = null) 
	{
    	...
    	if( $image_type == IMAGETYPE_JPEG ) 
    	{
      	...
      	elseif( $image_type == IMAGETYPE_PNG ) 
      	{
 			...
 			imagepng($this->image, $filename, 9, /*PNG_NO_FILTER*/PNG_ALL_FILTERS);
      	}
      	if( $permissions != null) 
      	{
	...
	/**
	 * Resize the current image resource
	 * 
	 * @param int $width
	 * @param int $height
	 */
	...
	/**
	 * Get the image buffer coded to base64
	 * 
	 * @return string
	 */
