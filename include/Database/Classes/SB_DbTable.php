...
class SB_DbTable extends SB_Object
{
	...
	const	TYPE_BIGINTEGER	= 12;
    const	TYPE_DECIMAL	= 13;
	const	TYPE_FLOAT		= 14;
	const	TYPE_STRING		= 15;
	const	TYPE_VARCHAR	= 16;
	const	TYPE_CHAR		= 17;
	const	TYPE_TEXT		= 18;
	const	TYPE_BLOB		= 19;
	const	TYPE_DATE		= 20;
	const	TYPE_DATETIME	= 21;
	const	TYPE_BIGINT     = 22;
	const 	MIN_DATETIME	= '0001-01-01 00:00:00';
	...
	public static function GetTable($table, $build_class = false, $dbh = null)
	{
		...
		if( !class_exists($class) && $build_class )
		{
			$cols = array();
			$key = null;
			$columns = self::GetTableColumns($table, false, $dbh);
			foreach($columns as $col)
			{
				if( $dbh->db_type == 'mysql' )
				{
					$type = 'null';
					if( $col->Type == 'date' )
						$type = 'SB_DbTable::TYPE_DATE';
					if( $col->Type == 'datetime' )
						$type = 'SB_DbTable::TYPE_DATE';
					if( $col->Type == 'text' )
						$type = 'SB_DbTable::TYPE_TEXT';
					if( strstr($col->Type, 'decimal') )
						$type = 'SB_DbTable::TYPE_DECIMAL';
					if( strstr($col->Type, 'tinyint') )
						$type = 'SB_DbTable::TYPE_TINYINY';
					if( strstr($col->Type, 'int') )
						$type = 'SB_DbTable::TYPE_INTEGER';
					if( strstr($col->Type, 'varchar') || strstr($col->Type, 'char') )
						$type = 'SB_DbTable::TYPE_VARCHAR';
						
					$cols[]= "'{$col->Field}' => array('type' => $type)";
					if( $col->Key == 'PRI' )
						$key = $col->Field;
				}
				elseif( $dbh->db_type == 'sqlite' || $dbh->db_type == 'sqlite3' )
				{
					...
					if( stristr($col->type, 'datetime') )
						$type = 'SB_DbTable::TYPE_DATETIME';
					if( stristr($col->type, 'date') )
						$type = 'SB_DbTable::TYPE_DATE';
					if( stristr($col->type, 'TEXT') )
						$type = 'SB_DbTable::TYPE_TEXT';
					if( strstr($col->type, 'decimal') )
						$type = 'SB_DbTable::TYPE_DECIMAL';
					if( stristr($col->type, 'int') )
						$type = 'SB_DbTable::TYPE_INTEGER';
					if( stristr($col->type, 'varchar') || strstr($col->type, 'char') )
						$type = 'SB_DbTable::TYPE_VARCHAR';
						
					$cols[] = "'{$col->name}' => array('type' => $type)";
					if( (int)$col->pk )
					{
						$key = $col->name;
					}
			...
			$code = "use SinticBolivia\SBFramework\Database\Classes\SB_DbTable;
			class $class extends SB_DbTable
			{
				protected function __construct(\$dbh = null)
				{
					...
					\$this->columns = array(". implode(',', $cols) .");
	...
	{
		$cols = $this->dbh->SanitizeColumns(array_keys($this->columns), $this->tableAlias);
	...
	public function GetRow($id, $class = 'SB_DbRow')
	{
		...
		if( $row && $class == 'SB_DbRow' && !is_a($row, 'SB_DbRow') ) 
		{
	...
	{
		if( !$column || !isset($this->columns[$column]) )
			return null;
    ...
    public static function GetType($type)
    {
        ...
        return constant("self::TYPE_$type");
    }
}
