...
use SinticBolivia\SBFramework\Database\SB_ResultSetSQLite;
...
class SB_Sqlite3 extends SB_Database
{
	...
	public function Query($query)
	{
		...
		if( preg_match('/(update|delete)/isU', $query) )
		{	
			$this->result = null;
			$res = $this->dbh->exec($query);
			if( !$res )
    ...
    /**
     * @param string $class The class name for resulset records
     * @return SB_ResultSetSQLite
     */
    public function GetResultSet($class = null)
    {
        $resultSet = new SB_ResultSetSQLite($this->result);
